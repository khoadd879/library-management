...
const AllBooks = () => {
  ...
  const [typeBooksWithCount, setTypeBooksWithCount] = useState<
    Array<{ idTypeBook: string; nameTypeBook: string; bookCount: number }>
  >([]); // TypeBooks từ API với count
    ...
    if (search) {
      ...
      result = result.filter(
        (book) =>
          ...
          book.authors?.some((a) =>
            a.nameAuthor.toLowerCase().includes(searchLower),
          ),
      );
    }
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-screen bg-gradient-to-br from-[#f4f7f9] to-[#e8f5f0] px-4 md:px-12 py-6 w-full"
    >
      {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/70 text-sm">
                {processedBooks.length} cuốn sách{" "}
                {selectedTypes.length > 0 &&
                  `trong ${selectedTypes.length} thể loại`}
              </p>
            </div>
          </div>
