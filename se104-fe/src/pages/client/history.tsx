                        ...
                        <div>
                            <div className="flex items-center gap-4 mb-4">
                                <div className="p-3 bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg">
                                    <BookOutlined className="text-emerald-300 text-2xl" />
                                </div>
                                <div>
                            ...
                            <div className="flex gap-4 mt-6">
                                <div className="px-4 py-2 bg-white/10 backdrop-blur-md rounded-xl border border-white/10">
                                    <span className="text-emerald-300 font-black text-xl">{history.length}</span>
                                ...
                                <div className="px-4 py-2 bg-amber-500/20 backdrop-blur-md rounded-xl border border-amber-400/20">
                                    <span className="text-amber-300 font-black text-xl">{history.filter(h => !h.isReturned).length}</span>
                                ...
                                <div className="px-4 py-2 bg-emerald-500/20 backdrop-blur-md rounded-xl border border-emerald-400/20">
                                    <span className="text-emerald-300 font-black text-xl">{history.filter(h => h.isReturned).length}</span>
            ...
            <div className="max-w-6xl mx-auto px-4 -mt-8 relative z-20 mb-8">
                <div className="bg-white rounded-2xl shadow-xl p-2 inline-flex gap-1">
                    {[
                        ...
                        { key: 'borrowing', label: `Đang mượn (${history.filter(h => !h.isReturned).length})` },
                        { key: 'returned', label: `Đã trả (${history.filter(h => h.isReturned).length})` },
                    ].map((tab) => (
                        <button
                            key={tab.key}
                                ...
                                >
                                    <div className="flex gap-4">
                                        ...
                                        <div className="w-20 h-28 flex-shrink-0 rounded-xl overflow-hidden shadow-lg relative">
                                            ...
                                            {/* Status overlay */}
                                            {!item.isReturned && (
                                                <div className={`absolute bottom-0 left-0 right-0 py-1 text-center text-[10px] font-black text-white ${
                                    ...
                                    {/* Timeline & Progress */}
                                    <div className={`rounded-xl p-4 space-y-3 mt-auto ${
                                        isOverdue ? 'bg-red-50' : item.isReturned ? 'bg-emerald-50/50' : 'bg-slate-50'
                                    }`}>
                                        <div className="flex justify-between items-center">
                                            <div className="text-center">
                                                <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Ngày mượn</p>
                                                ...
                                                <div className="flex-1 h-0.5 bg-slate-200 rounded-full relative">
                                                    <div 
                                                        className={`absolute left-0 top-0 h-full rounded-full transition-all duration-500 ${
                                                            isOverdue ? 'bg-red-400' : item.isReturned ? 'bg-emerald-400' : 'bg-amber-400'
                                                        }`}
                                                        style={{ width: item.isReturned ? '100%' : `${progress.percent}%` }}
                                                    />
                                                </div>
                                            </div>
                                            <div className="text-center">
                                                <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Hạn trả</p>
                                                <p className={`text-sm font-black mt-1 ${
                                                    isOverdue ? 'text-red-600' : item.isReturned ? 'text-emerald-600' : 'text-[#153D36]'
                                                }`}>
                                        ...
                                        {!item.isReturned && (
                                            <div className="pt-2">
                                                <Progress
                                                    ...
                                                    size="small"
                                                    showInfo={false}
                                                    strokeColor={isOverdue ? '#ef4444' : progress.percent >= 90 ? '#f59e0b' : '#10b981'}
