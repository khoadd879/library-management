...
import { message, Form, Select, InputNumber, Button, Card, Typography, Spin, Statistic, Row, Col, Divider } from "antd";
import { getAllReadersAPI, addPenaltyAPI } from "@/services/api";
import { UserOutlined, DollarOutlined, SolutionOutlined } from "@ant-design/icons";
...
const FineForm = () => {
  ...
  const fetchReaders = async () => {
    ...
    try {
      ...
      if (Array.isArray(res) && res.data[0].totalDebt > 0) {
        setReaders(res.data);
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="flex justify-center items-center min-h-[50vh] bg-gray-50 p-4">
      ...
      >
        <div className="text-center mb-6">
          ...
          <Text type="secondary">Thu hồi nợ phí trễ hạn từ độc giả</Text>
        </div>
        ...
        <Spin spinning={loading}>
          ...
          >
            {/* Chọn độc giả */}
            <Form.Item
              label={<span className="font-medium text-[#153D36]">Chọn độc giả</span>}
              name="idReader"
              ...
              >
                {readers.map((r) => (
                  <Option key={r.idReader} value={r.idReader}>
                    {r.nameReader} {r.totalDebt > 0 ? `(Nợ: ${r.totalDebt.toLocaleString()} đ)` : ''}
                  </Option>
                ))}
              </Select>
            </Form.Item>
            ...
            {form.getFieldValue('idReader') && currentDebt === 0 && (
                <div className="bg-green-50 p-3 rounded mb-4 text-green-700 flex items-center gap-2">
                    <SolutionOutlined /> Độc giả này không có khoản nợ nào.
                </div>
            )}

            {/* Nhập số tiền */}
            <Form.Item
              label={<span className="font-medium text-[#153D36]">Số tiền thu</span>}
              name="amount"
              rules={[
                ...
                },
                {
                    validator: async (_, value) => {
                        if (value > currentDebt) {
                            return Promise.reject(new Error("Không thể thu quá số tiền nợ!"));
                        }
                    }
                }
              ]}
            >
              <InputNumber
                ...
                onChange={(value) => setPayAmount(Number(value))}
