...
// 1. Import thêm icon cho logo mới
...
const UserSidebar: React.FC<UserSidebarProps> = ({
    ...
    return (
      <>
        ...
        <nav className="fixed top-0 left-0 right-0 h-16 bg-gradient-to-r from-[#153D36] via-[#1A4A42] to-[#153D36] text-white z-50 shadow-lg flex items-center justify-between px-4">
          {/* 2. Thay thế Logo Mobile */}
          <div
            className="flex items-center gap-2 cursor-pointer"
            onClick={() => navigate("/")}
          >
            <div className="w-8 h-8 bg-gradient-to-br from-[#0D2621] to-emerald-500 rounded-lg flex items-center justify-center shadow-md shadow-black/20">
  ...
  return (
    ...
    >
      ...
      {/* Navigation Items */}
      <ul
        className={`relative flex-1 py-6 space-y-1.5 px-3 no-scrollbar ${open ? "overflow-y-auto" : "overflow-hidden"}`}
      >
        {menuItems.map((item, index) => (
          <li key={index}>
            <button
      ...
      {/* 3. Thay thế Logo Desktop (khi mở sidebar) */}
      <div
        className={`flex items-center justify-center gap-3 py-15 border-t border-white/10 cursor-pointer hover:bg-white/5 transition-colors ${!open && "hidden"}`}
        onClick={() => navigate("/")}
      >
        <div className="w-13 h-13 bg-gradient-to-br from-[#0D2621] to-emerald-600 rounded-xl flex items-center justify-center shadow-lg shadow-black/30">
